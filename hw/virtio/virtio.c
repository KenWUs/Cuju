...
void virtqueue_mark_dirty(struct iovec *sg, hwaddr *addr, size_t num_sg)
{
    ...
    for (i = 0; i < num_sg; i++) {
        len = sg[i].iov_len;
        for (j = 0; j < len; j += 4096)
            kvm_shmem_mark_page_dirty(sg[i].iov_base + j, (addr[i] + j) >> TARGET_PAGE_BITS);
    }
}
...
static int virtio_blk_get(QEMUFile *f, void *opaque, size_t size)
{
    ...
    return virtio_load_blk(vdev, f, dc->vmsd->version_id);
}
...
int virtio_load_blk(VirtIODevice *vdev, QEMUFile *f, int version_id)
{
    ...
    /* Subsections */
    ret = virtio_blk_load_index(f, &vmstate_virtio, vdev, 1);
    if (ret) {
        return ret;
    }
